...
CREATE table  IF NOT EXISTS user_identity (
    ...
    provider			VARCHAR(50) NOT NULL,       -- vd: 'google', 'microsoft', 'github'
    provider_user_id 	VARCHAR(255) NOT NULL, -- ID từ provider
    access_token 		TEXT,                   -- if need save
    refresh_token 		TEXT,                  -- if need save
    created_at 			TIMESTAMP DEFAULT NOW(),
    updated_at 			TIMESTAMP DEFAULT NOW(),
    UNIQUE(provider, provider_user_id)   -- Unique 
);
